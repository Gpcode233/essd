  ...
  return (
    <div className="space-y-8">
        ...
        {filteredMatches.map((m) => {
          ...
          return (
            <div
              ...
              className="bg-essd-charcoal border-2 border-essd-border hover:border-essd-gold p-6 shadow-[4px_4px_0px_#0A0A0C] hover:shadow-[6px_6px_0px_#E8A927] hover:-translate-y-1 transition-all duration-200 cursor-pointer group flex flex-col justify-between rounded-2xl"
            >
              {/* Card Header */}
              <div>
                <div className="flex items-center justify-between pb-3 mb-4 border-b border-essd-border/80">
                  <div className="flex items-center gap-2">
                    <span className="px-2.5 py-0.5 bg-essd-gold text-essd-black font-mono text-xs font-black uppercase rounded-full">
                  ...
                  <div>
                    {isLive ? (
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-essd-orange text-white text-[10px] font-mono font-black uppercase rounded-full animate-pulse">
                        <span className="w-1.5 h-1.5 rounded-full bg-white"></span>
                        LIVE
                      </span>
                    ) : isFinal ? (
                      <span className="px-2.5 py-0.5 bg-green-950 text-green-300 border border-green-700 font-mono text-[10px] font-bold uppercase rounded-full">
                        FINAL RESULT
                      </span>
                    ) : (
                      <span className="px-2.5 py-0.5 bg-essd-dark text-essd-cream-muted font-mono text-[10px] uppercase rounded-full">
                        UPCOMING
                      </span>
                    )}
                  </div>
                ...
                <div className="space-y-3 mb-4">
                  {/* School A */}
                  <div
                    className={`p-3 border flex items-center justify-between transition-colors rounded-xl ${
                      isWinnerA
                ...
                {/* Motion snippet */}
                <div className="p-3 bg-essd-dark/80 border-l-3 border-essd-gold text-xs text-essd-cream font-sans italic mb-4 rounded-xl">
                  “{m.motionTopic}”
                </div>
              </div>
      ...
      {filteredMatches.length === 0 && (
        <div className="p-12 text-center bg-essd-charcoal border-2 border-dashed border-essd-border rounded-2xl">
